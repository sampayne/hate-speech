...
import zhang_util as util
import zhang_nlp as nlp
import zhang_text_preprocess as tp
import zhang_dnn_model_creator as dmc
...
def grid_search_dnn(model_descriptor: str,
                    cpus,
                    nfold,
                    X_train,
                    y_train,
    ...
    model = KerasClassifier(build_fn=create_model_with_args, verbose=0, batch_size=100, nb_epoch=10)
    ...
    _classifier = GridSearchCV(estimator=model,
    ...
    if (X_test is not None):
        ...
        util.save_scores(nfold_texts=X_train,
                         nfold_predictions=nfold_predictions,
                         nfold_truth=y_train,
                         heldout_texts=X_test,
                         heldout_predictions=heldout_predictions_final,
                         heldout_truth=y_test,
                         model_descriptor=model_descriptor,
                         digits=3,
...
def gridsearch(raw_data,
               labels,#input_data_file,
               model_descriptor: str,
               word_norm_option,
               randomize_strategy,
               pretrained_embedding_models=None,
    ...
    for text, label in raw_data:
        ...
        raw_labels.append(label)#labels.index(label) + 1)

    print('Pre-Processing Raw Data...')

    ##raw_data = pd.read_csv(input_data_file, sep=',', encoding="utf-8")
    M = get_word_vocab(raw_tweets, #raw_data.tweet,
                       word_norm_option)
    ...
    X_train_data, X_test_data, y_train, y_test = train_test_split(M0,
                     raw_labels,
                     #raw_data['class'],
                     #col_datasource,
    ...
    #print("Test Data:")
    #output_data_stats(X_test_data, y_test)

    # exit(0)

    classifier = grid_search_dnn(model_descriptor=model_descriptor,
                    cpus=CPUS,
                    nfold=5,
                    ...
                    X_test=X_test_data,
                    y_test=y_test,
                    embedding_layer_max_index=len(M[1]),
                    pretrained_embedding_matrix=pretrained_word_matrix,
                    word_dist_matrix=word_dist_matrix
                    )

    M0 = sequence.pad_sequences(M0, maxlen=MAX_SEQUENCE_LENGTH)

    final_preprocessed_data = []

    for i in range(0, len(M0)):
        final_preprocessed_data.append((M0[i], raw_labels[i]))

    return classifier, M0
